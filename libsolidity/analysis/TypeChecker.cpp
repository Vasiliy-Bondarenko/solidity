			...
			if (var.referenceLocation() != VariableDeclaration::Location::Storage)
			{
				if (!isLibraryFunction && _function.isPublic())
					m_errorReporter.typeError(
						3442_error,
						var.location(),
						"Types containing (nested) mappings can only have a data location of \"storage\" "
						"and thus only be parameters or return variables for internal or library functions."
					);
			}
			else
